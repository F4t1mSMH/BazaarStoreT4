...
public class DeleteProductSteps {
    ...
    public DeleteProductSteps(MangerScenarioContext context) {
    ...
    public void aProductIsCreatedSuccessfullyViaAPIAndItsIDIsAvailable() {
        ...
        Assert.assertTrue("Precondition failed: Product ID not retrieved.", context.productId > 0);
        System.out.println("Precondition: Product created for UI deletion test with ID: " + context.productId);
    }
    ...
    public void aDELETERequestIsSentToDeleteTheProductViaAPI() {
        response = given(ApiUtilities.spec())
                .delete("/products/" + context.productId);
    ...
    public void theProductIsDeletedSuccessfullyViaAPI() {
        ...
        System.out.println("Product ID " + context.productId + " successfully deleted and verified 404.");
    }
    ...
    public void theStoreManagerClicksTheDeleteButtonForTheProduct() {
        productsPage.clickDeleteButtonByName(context.productToVerifyName);
    }
